...
N* Tas<N>::outMin(){
    ...
    if (tas.size() > 1){
        ...
        int nextToCompare = tas[1] < tas[2] ? 1 : 2;
        while(nextToCompare < tas.size()){
            if (tas[curNode] > tas[nextToCompare]){
                ...
                if (tas.size() > nextToCompare * 2 + 2){
                    nextToCompare = tas[nextToCompare * 2 + 1] < tas[nextToCompare * 2 + 2] ? 
                        nextToCompare * 2 + 1 
                    : 
                        nextToCompare * 2 + 2;
...
template<class N>
std::ostream& operator << (std::ostream &os, const Tas<N> &tas){
